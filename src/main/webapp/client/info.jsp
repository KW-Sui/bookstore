...
<head>
	...
	<script>
		function checkpnum(id,pnum) {
			/*将pnum类型转换成int，否则可能会报错*/
			pnum = parseInt(pnum);
			/*库存判断*/
			if(pnum <= 0){
				alert("当前商品库存不足！");
			}else{
				window.location.href = "${pageContext.request.contextPath}/client/cart/addCart?id=${p.id}";
			}
		}
	</script>
</head>
		...
		<table width="100%" border="0" cellspacing="0">
			<tr>
				<td><div style="text-align:right; margin:5px 10px 5px 0px">
						...
						<a href="${pageContext.request.contextPath}/client/product/findProductByCategory?category=${p.category}">&nbsp;${p.category}</a>&nbsp;&nbsp;&nbsp;&nbsp;&gt;&nbsp;&nbsp;&nbsp;&nbsp;${p.name}
					</div>
