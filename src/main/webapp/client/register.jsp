...
<script type="text/javascript">
    function usernameTest() {
        $.post(
            "${pageContext.request.contextPath}/client/user/usernameTest?username=" + $("#username").val(),
            function (data) {
                if(data == "EXIST"){
                    alert("会员名已存在，请重新输入！");
                    $("#username").val("");
...
function emailTest() {
	$.post(
			"${pageContext.request.contextPath}/client/user/emailTest?email=" + $("#email").val(),
			function (data) {
				if(data == "EXIST"){
					alert("该邮箱已被注册，请重新输入！");
					$("#email").val("");
				...
				<tr>
					<td style="padding: 30px">
						<c:if test="${requestScope.register_error} != null">
							<h1>${requestScope.register_error}</h1>
						</c:if>
						<c:if test="${requestScope.register_error} == null">
							<h1>新会员注册</h1>
						</c:if>
						<table width="70%" border="0" cellspacing="2" class="upline">
							<tr>
								...
								<td style="width: 40%">
								<input type="text" class="textinput"  id="email" name="email" onkeyup="checkEmail();" onblur="emailTest()"/>
								</td>
								<td colspan="2"><span id="emailMsg"></span><font color="#999999">请输入有效的邮箱地址</font></td>
							</tr>
							<tr>
								<td style="text-align: right">会员名：</td>
								<td><input type="text" class="textinput"  id="username" name="username" onkeyup="checkUsername();" onblur="usernameTest()"/>
								</td>
						...
						<table width="80%" border="0" cellspacing="2" class="upline">
							<tr>
								...
								<td style="color: red">${requestScope.check_error}</td>
							</tr>
							<tr>
